...
export default function Carditems() {
  ...
  return (
    <Box 
      ...
      maxWidth="1980px" 
      m="auto"
      sx={{ gap: 4 }} // Add gap between product listing and cart summary
    >
      {/* Product Listing */}
      ...
      <Box
        ...
        bgcolor="#fafafa"
      >
        <Typography variant="h6" fontWeight="bold" color="#964c39">
          Your Cart ({cart.reduce((acc, item) => acc + (item.quantity ?? 0), 0)})
        </Typography>
        {cart.map(cartItem => (
          <Box key={cartItem.id} display="flex" alignItems="center" justifyContent="space-between" width="100%" my={1}>
            ...
            <Typography>${(cartItem.price * (cartItem.quantity ?? 1)).toFixed(2)}</Typography>
          </Box>
        ))}
